#!/usr/bin/env node
// flint init — drop the always-on flint activation rule into a target
// repo for every IDE agent we support. Idempotent. Safe to re-run.
//
// Usage:
//   node src/tools/flint-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
//   curl -fsSL https://raw.githubusercontent.com/Marcelover777/flint/main/src/tools/flint-init.js | node - [args]
//
...
// existing memory files — that's the job of `/flint:compress`.

const fs = require('fs');
const path = require('path');

// Embedded so the tool works standalone (npx-style) without the src/rules/ dir.
// Mirrors src/rules/flint-activate.md verbatim — keep these in sync.
const RULE_BODY = `Respond terse like smart flint. All technical substance stay. Only fluff die.
...
Switch level: /flint lite|full|ultra|wenyan
Stop: "stop flint" or "normal mode"

Auto-Clarity: drop flint for security warnings, irreversible actions, user confused. Resume after.

Boundaries: code/commits/PRs written normal.
`;

const SENTINEL = 'Respond terse like smart flint';
...
// lives at bin/lib/openclaw.js; we require it lazily so flint-init.js
...
const AGENTS = [
  { id: 'cursor',   file: '.cursor/rules/flint.mdc',
    frontmatter: '---\ndescription: "Flint mode — terse communication, ~75% fewer tokens, full technical accuracy"\nalwaysApply: true\n---\n\n',
    mode: 'replace' },
  { id: 'windsurf', file: '.windsurf/rules/flint.md',
    frontmatter: '---\ntrigger: always_on\n---\n\n',
    mode: 'replace' },
  { id: 'cline',    file: '.clinerules/flint.md',
    frontmatter: '',
    mode: 'replace' },
  ...
  { id: 'openclaw', description: '~/.openclaw/workspace/{skills/flint/, SOUL.md}',
    installer: 'openclaw' },
];
...
function loadRuleBody() {
  ...
  try {
    const local = path.join(__dirname, '..', 'rules', 'flint-activate.md');
...
function help() {
  console.log(`flint init — drop always-on flint rule into a target repo

Usage: flint-init.js [target-dir] [--dry-run] [--force] [--only <agent>]
...
function main() {
  ...
  console.log(`⚡ flint init — ${opts.target}${opts.dryRun ? ' (dry run)' : ''}\n`);
