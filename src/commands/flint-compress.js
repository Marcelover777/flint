...
const { loadConfig } = require('../hooks/flint-config');
...
function usage() {
  return `Usage: node src/commands/flint-compress.js <file> [flags]
...
Flags:
  ...
  --restore        restore latest .flint backup or legacy .original.md
...
async function compressFile(opts) {
  ...
  const cachePath = path.join(path.dirname(io.target), '.flint', 'cache', 'compress-v1.json');
  ...
  const reportDir = path.join(path.dirname(io.target), '.flint', 'reports');
  ...
  if (!opts.check && !opts.dryRun) {
    const backupDir = path.join(path.dirname(io.target), '.flint', 'backups', new Date().toISOString().replace(/[:.]/g, '-'));
...
async function main() {
  try {
    ...
    if (opts.restore) {
      ...
      const backupRoot = path.join(path.dirname(file), '.flint', 'backups');
