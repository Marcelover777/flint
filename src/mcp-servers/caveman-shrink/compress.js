// caveman-shrink prose compressor. Uses repo core modules when available and
...
function fallbackProtected(text, transform) {
  ...
  for (const re of PROTECTED_PATTERNS) {
    working = working.replace(re, m => {
      ...
      segments.push(m);
      return `__CAVEMAN_MCP_${i}__`;
    });
  }
  let out = transform(working);
  out = out.replace(/__CAVEMAN_MCP_(\d+)__/g, (_, i) => segments[Number(i)]);
  return out;
}
