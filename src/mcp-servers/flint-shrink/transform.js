  ...
  for (const item of items) {
    ...
    for (const field of fields) {
      ...
      if (after !== before) {
        item[field] = after;
        changed++;
        if (debug) process.stderr.write(`[flint-shrink] ${method}.${item.name || '?'} ${field}: ${before.length}->${after.length}\n`);
      }
    }
  }
  return changed;
...
function transformToolResult(message, opts = {}) {
  ...
  if (changed > 0 && !validateToolResult(before, after)) {
    if (opts.debug) process.stderr.write('[flint-shrink] tools/call result validation failed; passing original\n');
    return message;
  }
  return after;
}
...
function transformResponse(message, method, opts = {}) {
  ...
  if (changed > 0 && !validateTransform(before, after, { preserveInputSchema: opts.preserveInputSchema !== false })) {
    if (opts.debug) process.stderr.write(`[flint-shrink] validation failed for ${method}; passing original\n`);
    return message;
  }
