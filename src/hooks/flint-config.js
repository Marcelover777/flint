#!/usr/bin/env node
// flint — shared configuration resolver
//
// Resolution order for default mode:
//   1. FLINT_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/flint/config.json (any platform, if set)
//      - ~/.config/flint/config.json (macOS / Linux fallback)
//      - %APPDATA%\flint\config.json (Windows fallback)
//   3. 'full'
...
const DEFAULT_CONFIG = {
  ...
  // FLINT_TARGET_MODEL or config if your sessions run another model.
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'flint');
  }
  ...
  return path.join(os.homedir(), '.config', 'flint');
}
...
function envConfig() {
  ...
  if (process.env.FLINT_SHRINK_FIELDS) {
    out.mcpShrink = {
      ...(out.mcpShrink || {}),
      fields: process.env.FLINT_SHRINK_FIELDS.split(',').map(s => s.trim()).filter(Boolean),
    };
  }
  if (process.env.FLINT_SHRINK_NESTED_SCHEMA) {
    out.mcpShrink = {
      ...(out.mcpShrink || {}),
