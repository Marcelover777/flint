...
function formatShare({ inputTokens = 0, outputTokens, cacheReadTokens = 0, cacheCreationTokens = 0, turns, mode, model }) {
  ...
  if (s.ratio !== null) {
    ...
    return `⚡ Saved ${s.estimatedSavedOutput.toLocaleString()} output tokens${usd} across ${turns} turns this session`;
  }
...
function jsonPayload({ parsed, mode, sessionFile, compressed }) {
  ...
  return {
    ...
    notes: [
      ...
      'Use /flint-bench --model claude-opus-4-8 to calibrate.',
    ],
  };
}
