...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag, getTargetModel } = require('./flint-config');
...
function main() {
  ...
  const historyPath = path.join(claudeDir, '.flint-history.jsonl');
  ...
  const mode = readFlag(path.join(claudeDir, '.flint-active'));
  ...
  if (parsed.turns > 0) {
    ...
    safeWriteFlag(path.join(claudeDir, '.flint-statusline-suffix'), suffix);
  }
