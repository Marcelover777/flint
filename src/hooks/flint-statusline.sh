#!/bin/bash
# flint — statusline badge script for Claude Code
# Reads the flint mode flag file and outputs a colored badge.
#
# Usage in ~/.claude/settings.json:
#   "statusLine": { "type": "command", "command": "bash /path/to/flint-statusline.sh" }
#
# Plugin users: Claude will offer to set this up on first session.
# Standalone users: install.sh wires this automatically.

FLAG="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.flint-active"
...
if [ -z "$MODE" ] || [ "$MODE" = "full" ]; then
  printf '\033[38;5;172m[FLINT]\033[0m'
else
  SUFFIX=$(printf '%s' "$MODE" | tr '[:lower:]' '[:upper:]')
  printf '\033[38;5;172m[FLINT:%s]\033[0m' "$SUFFIX"
fi

# Savings suffix: on by default. Opt out via FLINT_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by flint-stats.js so we don't shell out
...
# ANSI escape codes otherwise). Until /flint-stats has run at least once,
...
if [ "${FLINT_STATUSLINE_SAVINGS:-1}" != "0" ]; then
  SAVINGS_FILE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.flint-statusline-suffix"
