# flint — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/Marcelover777/flint/main/src/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the flint hooks (used to merge" -ForegroundColor Red
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and (Test-Path $Settings)) {
        try {
                ...
                foreach ($entry in $entries) {
                    if ($entry.hooks) {
                        foreach ($hookDef in $entry.hooks) {
                            if ($hookDef.command -and $hookDef.command.Contains("flint")) {
                                return $true
                            }
                        }
                    }
            ...
            $HooksWired = (& $hasFlintHook "SessionStart") -and (& $hasFlintHook "UserPromptSubmit")
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "Flint hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "flint-activate.js"))) {
    Write-Host "Reinstalling flint hooks (-Force)..."
} else {
    Write-Host "Installing flint hooks..."
}
...
$tmpScript = Join-Path $env:TEMP "flint-install-$([System.Diagnostics.Process]::GetCurrentProcess().Id).js"
try {
...
Write-Host "  - SessionStart hook: auto-loads flint rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/flint lite, /flint ultra, /flint-commit, etc.)"
Write-Host "  - Statusline badge: shows [FLINT] or [FLINT:ULTRA] etc."
