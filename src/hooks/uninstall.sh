#!/bin/bash
# flint — uninstaller for the SessionStart + UserPromptSubmit hooks
# Removes: hook files in ~/.claude/hooks, settings.json entries, and the flag file
# Usage: bash src/hooks/uninstall.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/Marcelover777/flint/main/src/hooks/uninstall.sh)
set -e

CLAUDE_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
HOOKS_DIR="$CLAUDE_DIR/hooks"
SETTINGS="$CLAUDE_DIR/settings.json"
FLAG_FILE="$CLAUDE_DIR/.flint-active"
...
# Detect if flint is installed as a plugin (check plugin cache)
PLUGIN_INSTALLED=0
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/flint*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi

if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  echo "Flint appears to be installed as a Claude Code plugin."
  echo "To uninstall the plugin, run:"
  echo ""
  echo "  claude plugin disable flint"
  echo ""
...
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the flint SessionStart and UserPromptSubmit"
    ...
    FLINT_SETTINGS="$SETTINGS" FLINT_HOOKS_DIR="$HOOKS_DIR" node -e "
      ...
      // Remove statusLine if it references flint
...
echo ""
echo "Other agents:"
echo "  npx skills remove flint    # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable flint  # Claude Code plugin"
echo "  gemini extensions uninstall flint  # Gemini CLI"
