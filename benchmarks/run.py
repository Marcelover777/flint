#!/usr/bin/env python3
"""Benchmark flint vs normal Claude output token counts."""
...
SKILL_PATH = REPO_DIR / "skills" / "flint" / "SKILL.md"
...
def run_benchmarks(client, model, prompts, flint_system, trials):
    results = []
    total = len(prompts)
    ...
    for i, prompt_entry in enumerate(prompts, 1):
        ...
        for mode, system in [("normal", NORMAL_SYSTEM), ("flint", flint_system)]:
...
def compute_stats(results):
    ...
    for entry in results:
        ...
        flint_medians = statistics.median(
            [t["output_tokens"] for t in entry["flint"]]
        )
        savings = 1 - (flint_medians / normal_medians) if normal_medians > 0 else 0
        ...
        rows.append(
            {
                ...
                "flint_median": int(flint_medians),
    ...
    avg_flint = round(statistics.mean([r["flint_median"] for r in rows]))
...
def format_table(rows, summary):
    lines = [
        "| Task | Normal (tokens) | Flint (tokens) | Saved |",
        "|------|---------------:|----------------:|------:|",
    ]
    for r in rows:
        label = format_prompt_label(r["id"])
        lines.append(
            f"| {label} | {r['normal_median']} | {r['flint_median']} | {r['savings_pct']}% |"
        )
    lines.append(
        f"| **Average** | **{summary['avg_normal']}** | **{summary['avg_flint']}** | **{summary['avg_savings']}%** |"
    )
    lines.append("")
    lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark flint vs normal Claude")
    ...
    flint_system = load_flint_system()
    ...
    results = run_benchmarks(client, args.model, prompts, flint_system, args.trials)
