...
$StateDir = if ($env:FLINT_STATE_DIR) { $env:FLINT_STATE_DIR } else { Join-Path (Split-Path $PSScriptRoot -Parent) "state" }
$Flag = Join-Path $StateDir ".flint-active"
...
if ([string]::IsNullOrEmpty($Mode) -or $Mode -eq "full") {
    [Console]::Write("${Esc}[38;5;214m[FLINT]${Esc}[0m")
} else {
    $Suffix = $Mode.ToUpperInvariant()
    [Console]::Write("${Esc}[38;5;214m[FLINT:$Suffix]${Esc}[0m")
}

# Savings suffix: on by default. Opt out via FLINT_STATUSLINE_SAVINGS=0.
# Reads a pre-rendered string written by flint-stats.js on each /flint-stats
# run. Refuses reparse points and strips control bytes. Until /flint-stats has
# run at least once the suffix file is absent and nothing renders — no fake
# numbers on fresh installs.
if ($env:FLINT_STATUSLINE_SAVINGS -ne "0") {
    $SavingsFile = Join-Path $StateDir ".flint-statusline-suffix"
        ...
        try {
            ...
            if (-not ($SavingsItem.Attributes -band [System.IO.FileAttributes]::ReparsePoint) -and
                ...
                if ($Savings.Length -gt 0) {
                    [Console]::Write(" ${Esc}[38;5;214m$Savings${Esc}[0m")
                }
            }
        } catch {}
    }
