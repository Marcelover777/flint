#!/usr/bin/env node
// flint-stats - read Claude Code session logs, print token usage, and
// estimate savings/costs with model-aware pricing (Fable 5 default; prices by
// the model recorded in the session log, any Claude via the central table).

const fs = require('fs');
const path = require('path');
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag, getTargetModel, getStateDir } = require('./flint-config');
...
function loadCompressionRatios() {
  try {
    const p = path.join(__dirname, '..', 'data', 'output-savings.json');
...
function formatHistory({ sessions, inputTokens = 0, outputTokens, cacheReadTokens = 0, cacheCreationTokens = 0, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nFlint Stats - Lifetime${window}\n${sep}\nNo sessions logged yet - run /flint-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ inputTokens = 0, outputTokens, cacheReadTokens = 0, cacheCreationTokens = 0, turns, mode, model }) {
  if (turns === 0) return '⚡ flint armed but no turns yet';
  ...
  if (s.ratio !== null) {
    ...
    return `⚡ flint saved ${s.estimatedSavedOutput.toLocaleString()} output tokens${usd} across ${turns} turns this session`;
  }
...
function formatStats({ inputTokens = 0, outputTokens, cacheReadTokens = 0, cacheCreationTokens = 0, turns, mode, model, sessionPath, compressed }) {
  ...
  if (s.ratio !== null) {
    const usdLine = s.savedCostUsd != null && s.savedCostUsd > 0
      ...
      : '';
    savings = `Est. without flint:  ${s.estimatedBaselineOutput.toLocaleString()}\n` +
              `Est. tokens saved:     ${s.estimatedSavedOutput.toLocaleString()} (~${Math.round(s.ratio * 100)}%)\n` +
              usdLine.replace(/\n$/, '');
    if (s.pricing) footer = `Savings est. from calibrated ratios (data/output-savings.json). Pricing for ${model}. Actual varies by task.`;
    else footer = 'Savings est. from calibrated ratios. Model pricing unknown; USD not calculated.';
  } else if (mode && mode !== 'off') {
    savings = `No savings estimate for '${mode}' mode - only 'full' has benchmark data.`;
  } else {
    savings = 'Flint not active this session.';
  }
...
function jsonPayload({ parsed, mode, sessionFile, compressed }) {
  ...
  return {
    ...
    notes: [
      ...
      'Use /flint-bench --model claude-fable-5 to calibrate.',
    ],
  };
}
...
function main() {
  ...
  // Transcripts live under the real Claude Code config dir; flint state
  // (history, flag, statusline suffix) lives in the project-local state dir.
  const claudeDir = process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude');
  const stateDir = getStateDir();
  const historyPath = path.join(stateDir, '.flint-history.jsonl');

  if (all || sinceArg) {
    const sinceMs = parseDuration(sinceArg);
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`flint-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  if (!sessionFile) {
    process.stderr.write('flint-stats: no Claude Code session found.\n');
    process.exit(1);
  }

  const parsed = parseSession(sessionFile);
  const mode = readFlag(path.join(stateDir, '.flint-active'));
