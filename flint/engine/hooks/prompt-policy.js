...
// Filler markers in the model's previous output, EN + PT-BR. High density means
// the terse register drifted and needs a reinforcement nudge.
const DRIFT_RE = /\b(sure|certainly|of course|happy to|i think|perhaps|maybe|basically|actually|com certeza|claro que|basicamente|na verdade|com prazer|certamente)\b/gi;
...
function reinforcementText(mode) {
  return 'FLINT ' + mode + ' still on: answer-first, no filler/recap/summary-close' +
    (mode === 'lite' ? ', keep grammar' : ', fragments OK') +
    '; short final, don\'t re-print code/diffs. Code/paths/numbers/errors exact. Plain prose for safety.';
}
...
function shouldReinforce({ prompt, activeMode, config, state, transcript }) {
  ...
  if (/^\/flint(?:-stats|-compress|-doctor|-bench|-config|-commit|-review|-help)?\b/i.test(prompt || '')) {
...
function decideReinforcement({ data, prompt, activeMode, config, claudeDir }) {
  ...
  const file = statePath(claudeDir || process.env.FLINT_STATE_DIR || path.resolve(__dirname, '..', 'state'), sessionId);
