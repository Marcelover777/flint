#!/usr/bin/env node
// flint-bench — offline token estimates plus an opt-in, budget-guarded
// online benchmark against the Anthropic API.
//
//   node .claude/flint/commands/flint-bench.js --offline --report
//   node .claude/flint/commands/flint-bench.js --online [--max-spend 1] [--model claude-fable-5] [--report]
//
...
// core/pricing.js, with a conservative pre-call worst-case guard.
const fs = require('fs');
...
// Engine root (.claude/flint) holds evals/; the skill lives one level up in
// .claude/skills/flint; .env.local (optional ANTHROPIC_API_KEY) at project root.
const ROOT = path.resolve(__dirname, '..');
const SKILL_DIR = path.resolve(ROOT, '..', 'skills', 'flint');
const HARD_CAP_USD = 15;
const DEFAULT_MAX_SPEND_USD = 1;
const ONLINE_MAX_TOKENS = 800;
loadLocalEnv({ root: path.resolve(ROOT, '..', '..') });
...
const V1_REFERENCE = {
  micro_full_line: 'FLINT full. Terse. Drop filler/pleasantry/hedge/articles. Fragments OK. Preserve code, ids, paths, URLs, numbers, errors exact. Normal prose for safety/destructive/ambiguous multi-step. Persist until off.',
...
function readMicroLine(label) {
  try {
    const content = fs.readFileSync(path.join(SKILL_DIR, 'MICRO.md'), 'utf8');
    const hit = content.split('\n').map(s => s.trim()).find(line => line.startsWith(`FLINT ${label}.`));
    if (hit) return hit;
  } catch (_) {}
...
function offlineReport() {
  ...
  return {
    ...
    arms: ['baseline', 'terse', 'flint-current-full', 'optimizer-micro-full', 'optimizer-adaptive-full', 'optimizer-lite', 'optimizer-ultra', 'local-compress-only', 'hybrid-compress'],
...
async function runDocCompressionSmoke({ model, pricing, guard, failures }) {
  const { compressFile, getLlmUsage, resetLlmUsage } = require('./flint-compress');
...
async function main() {
  try {
    ...
    process.stderr.write(`flint-bench: ${error.message}\n`);
    process.exitCode = 1;
  }
}
