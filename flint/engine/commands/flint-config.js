...
const { getConfigDir, getConfigPath, loadConfig, mergeConfig, normalizeConfig } = require('../hooks/flint-config');
...
function main() {
  ...
  if (cmd !== 'set' || !key || value == null) {
    process.stderr.write('Usage: node .claude/flint/commands/flint-config.js show|set <path> <value>\n');
    process.exit(2);
  }
  let current = {};
