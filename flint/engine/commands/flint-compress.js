...
const { loadConfig, getStateDir } = require('../hooks/flint-config');
...
const LLM_TIMEOUT_MS = Number(process.env.FLINT_LLM_TIMEOUT_MS || 60_000);
...
function usage() {
  return `Usage: node .claude/flint/commands/flint-compress.js <file> [flags]
...
Flags:
  ...
  --restore        restore latest engine-state backup or legacy .original.md
...
function buildCompressPrompt(maskedText, mode, repair) {
  const rules = [
    'Rewrite the text below as maximally terse flint-style technical notes, in the SAME language as the input.',
    'Hard rules:',
    ...
    '- Tokens matching __FLINT_PROTECTED_NNNNNN_hhhhhhhh__ are frozen content. Reproduce each one byte-exact, same count, same order. Never invent, drop, merge, split, or edit them.',
...
async function compressFile(opts) {
  ...
  // Cache, reports, and backups all live in the engine state dir so compressing
  // repo docs never drops a .flint/ folder into the working tree.
  const cachePath = path.join(STATE_DIR, 'cache', 'compress-v1.json');
  ...
  if (!opts.check && !opts.dryRun) {
    const backupDir = path.join(STATE_DIR, 'backups', new Date().toISOString().replace(/[:.]/g, '-'));
...
async function main() {
  try {
    ...
    if (opts.restore) {
      ...
      const backupRoot = path.join(STATE_DIR, 'backups');
    ...
    process.stderr.write(`flint-compress: ${error.message}\n`);
    process.exitCode = 1;
  }
}
