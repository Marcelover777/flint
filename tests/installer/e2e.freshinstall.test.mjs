...
function freshTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'flint-freshinstall-'));
}
...
function flintHookCommands(settings, event, marker) {
...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'flint-activate.js')),     'flint-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'flint-mode-tracker.js')), 'flint-mode-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'flint-config.js')),       'flint-config.js missing');
    ...
    assert.ok(SETTINGS.hasFlintHook(settings, 'SessionStart', 'flint-activate'),
      'SessionStart hook missing or wrong marker');
    assert.ok(SETTINGS.hasFlintHook(settings, 'UserPromptSubmit', 'flint-mode-tracker'),
      'UserPromptSubmit hook missing or wrong marker');
    assert.ok(settings.statusLine, 'statusLine not set');
    assert.match(getStatuslineCommand(settings), /flint-statusline/,
      'statusLine command does not reference flint');
  } finally {
...
test('idempotent install does not duplicate hook entries (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH' }, () => {
  ...
  try {
    ...
    const sessStart = flintHookCommands(settings, 'SessionStart', 'flint-activate');
    assert.equal(sessStart.length, 1, `expected 1 SessionStart flint hook, got ${sessStart.length}`);

    const ups = flintHookCommands(settings, 'UserPromptSubmit', 'flint-mode-tracker');
    assert.equal(ups.length, 1, `expected 1 UserPromptSubmit flint hook, got ${ups.length}`);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('uninstall strips flint hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['flint-activate.js', 'flint-mode-tracker.js', 'flint-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining flint-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /flint/, `${ev} still has flint hook: ${h.command}`);
        }
      }
    }
    // User's pre-existing hook preserved.
    const preservedUser = flintHookCommands(settings, 'SessionStart', 'user-owned-hook').length > 0;
    assert.ok(preservedUser, 'user-authored SessionStart hook was wiped during uninstall');

    // Statusline pointing at flint should be removed.
    assert.doesNotMatch(getStatuslineCommand(settings), /flint-statusline/,
      'flint statusline survived uninstall');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
`);
    ...
    // Flint hooks must be wired.
    assert.ok(SETTINGS.hasFlintHook(parsed, 'SessionStart', 'flint-activate'));
    assert.ok(SETTINGS.hasFlintHook(parsed, 'UserPromptSubmit', 'flint-mode-tracker'));
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'flint', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  try {
    ...
    const skillFile = path.join(ws, 'skills', 'flint', 'SKILL.md');
    ...
    assert.match(soulRaw, /<!-- flint-begin -->/, 'SOUL.md missing begin marker');
    assert.match(soulRaw, /<!-- flint-end -->/, 'SOUL.md missing end marker');
    assert.match(soulRaw, /Respond terse like smart flint/, 'SOUL.md missing sentinel');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const skillRaw = fs.readFileSync(path.join(ws, 'skills', 'flint', 'SKILL.md'), 'utf8');
    ...
    const beginMatches = soulRaw.match(/<!-- flint-begin -->/g) || [];
...
test('openclaw install preserves user content in SOUL.md (append, not overwrite)', () => {
  ...
  try {
    ...
    assert.match(soulRaw, /<!-- flint-begin -->/, 'flint block not appended');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('openclaw uninstall removes skill folder + strips SOUL.md block, preserving user content', () => {
  ...
  try {
    ...
    assert.equal(fs.existsSync(path.join(ws, 'skills', 'flint')), false, 'skill folder should be removed');
    const soulAfter = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.doesNotMatch(soulAfter, /<!-- flint-begin -->/, 'flint block survived uninstall');
    assert.doesNotMatch(soulAfter, /<!-- flint-end -->/, 'flint end marker survived uninstall');
...
test('flint-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
    const initScript = path.join(REPO_ROOT, 'src', 'tools', 'flint-init.js');
    const r = spawnSync('node', [initScript, dir, '--only', 'openclaw'], {
      env: { ...process.env, OPENCLAW_WORKSPACE: ws, NO_COLOR: '1' },
      encoding: 'utf8',
    });
    assert.equal(r.status, 0, `flint-init failed: ${r.stderr || r.stdout}`);
    assert.ok(fs.existsSync(path.join(ws, 'skills', 'flint', 'SKILL.md')), 'skill missing via init route');
    assert.ok(fs.existsSync(path.join(ws, 'SOUL.md')), 'SOUL.md missing via init route');
    const soulRaw = fs.readFileSync(path.join(ws, 'SOUL.md'), 'utf8');
    assert.match(soulRaw, /Respond terse like smart flint/, 'sentinel missing via init route');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/flint-activate.js"',
      marker: 'flint-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/flint-activate.js"',
      marker: 'flint-activate',
    });
