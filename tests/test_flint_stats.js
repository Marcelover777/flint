#!/usr/bin/env node
// Tests for /flint-stats — direct script invocation and via mode tracker.
// Run: node tests/test_flint_stats.js
...
const STATS = path.join(ROOT, 'src', 'hooks', 'flint-stats.js');
const TRACKER = path.join(ROOT, 'src', 'hooks', 'flint-mode-tracker.js');

let passed = 0;
let failed = 0;

function test(name, fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'flint-stats-test-'));
  try {
    fn(tmp);
    passed++;
    console.log(`  ✓ ${name}`);
...
test('skips estimate for non-full modes', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.flint-active'), 'ultra');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
...
test('mode tracker handles /flint-stats with decision block', (tmp) => {
  const sess = makeSession(tmp, [
    { type: 'assistant', message: { usage: { output_tokens: 100 } } },
  ]);
  const claudeDir = path.join(tmp, '.claude');
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    ...
    input: JSON.stringify({ prompt: '/flint-stats', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
  assert.match(parsed.reason, /Flint Stats/);
  assert.match(parsed.reason, /Output tokens:\s+100/);
});

test('mode tracker preserves flint flag when /flint-stats fires', (tmp) => {
  const sess = makeSession(tmp, [
  ...
  assert.strictEqual(fs.readFileSync(path.join(claudeDir, '.flint-active'), 'utf8'), 'full');
});
...
test('formatStats handles empty session gracefully', () => {
  const { formatStats } = require(path.join(ROOT, 'src', 'hooks', 'flint-stats.js'));
...
test('--share works with no benchmark ratio (lite mode)', (tmp) => {
  ...
  const claudeDir = path.join(tmp, '.claude');
  fs.writeFileSync(path.join(claudeDir, '.flint-active'), 'lite');
  const out = execFileSync(process.execPath, [STATS, '--session-file', sess, '--share'], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir },
  });
...
test('appends to lifetime history on each run', (tmp) => {
  ...
  const histPath = path.join(claudeDir, '.flint-history.jsonl');
...
test('skips pairs where compressed is not actually smaller', (tmp) => {
  const { findCompressedPairs } = require(path.join(ROOT, 'src', 'hooks', 'flint-stats.js'));
...
test('writes statusline suffix file after a stats run', (tmp) => {
  ...
  const suffixPath = path.join(claudeDir, '.flint-statusline-suffix');
...
test('humanizeTokens formats small/medium/large correctly', () => {
  const { humanizeTokens } = require(path.join(ROOT, 'src', 'hooks', 'flint-stats.js'));
...
test('statusline.sh appends savings when FLINT_STATUSLINE_SAVINGS=1', (tmp) => {
  if (process.platform === 'win32') return; // bash test
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.flint-active'), 'full');
  fs.writeFileSync(path.join(claudeDir, '.flint-statusline-suffix'), '⚡ 2.8k');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'flint-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, FLINT_STATUSLINE_SAVINGS: '1' },
  });
  assert.match(out, /\[FLINT\]/);
...
test('statusline.sh renders savings by default when env var is unset', (tmp) => {
  ...
  delete env.FLINT_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'flint-statusline.sh')], {
    encoding: 'utf8', env,
  });
  assert.match(out, /\[FLINT\]/);
...
test('statusline.sh omits savings when FLINT_STATUSLINE_SAVINGS=0', (tmp) => {
  if (process.platform === 'win32') return;
  const claudeDir = path.join(tmp, '.claude');
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.writeFileSync(path.join(claudeDir, '.flint-active'), 'full');
  ...
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'flint-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, FLINT_STATUSLINE_SAVINGS: '0' },
  });
  assert.match(out, /\[FLINT\]/);
...
test('statusline.sh omits savings when suffix file is missing (fresh install)', (tmp) => {
  ...
  // /flint-stats has run. Default-on must NOT fabricate a number.
  const env = { ...process.env, CLAUDE_CONFIG_DIR: claudeDir };
  delete env.FLINT_STATUSLINE_SAVINGS;
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'flint-statusline.sh')], {
    encoding: 'utf8', env,
...
test('statusline.sh strips control bytes from suffix', (tmp) => {
  ...
  fs.writeFileSync(path.join(claudeDir, '.flint-statusline-suffix'), '\x1b[31mEVIL');
  const out = execFileSync('bash', [path.join(ROOT, 'src', 'hooks', 'flint-statusline.sh')], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, FLINT_STATUSLINE_SAVINGS: '1' },
  });
...
test('appendFlag is symlink-safe (refuses symlinked target)', (tmp) => {
  ...
  const { appendFlag } = require(path.join(ROOT, 'src', 'hooks', 'flint-config.js'));
...
test('mode tracker forwards --share to stats script', (tmp) => {
  ...
  const out = execFileSync(process.execPath, [TRACKER], {
    encoding: 'utf8',
    env: { ...process.env, CLAUDE_CONFIG_DIR: claudeDir, HOME: tmp },
    input: JSON.stringify({ prompt: '/flint-stats --share', transcript_path: sess }),
  });
  const parsed = JSON.parse(out);
  assert.strictEqual(parsed.decision, 'block');
