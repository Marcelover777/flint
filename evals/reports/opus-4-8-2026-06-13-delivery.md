...
come from one budgeted run of `node src/commands/flint-bench.js --online --model
...
Optimizer emits ~43% fewer output tokens than the flint-style line
...
| Flint (output only, 59.4%) | 4,000 | 406 | $0.0905 |
| **Optimizer** (output 76.7% + docs ~46%) | 2,160 | 233 | **$0.0499** |

→ ~45% cheaper/turn than flint, ~63% cheaper than no compression; context
...
1. **Fable-5 dead-model defaults** (the real bug exposed by retirement):
   ...
   `claude-sonnet-4-6`; `flint-bench` online default, `flint-compress` bare
...
3. **`flint-bench` report filenames** now use the benched model slug
...
- `git diff --check` clean. `flint-doctor --json` exit 0.
