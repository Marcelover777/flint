...
description: >
  ...
  with local-first Flint optimizer. Preserves code, URLs, paths, commands,
  env vars, numbers, headings, frontmatter, and protected blocks exactly.
  Supports --check, --diff, --local-only, --llm, --restore, and --json.
  Trigger: /flint-compress FILEPATH or "compress memory file"
---
...
`/flint-compress <filepath>` or user asks to compress memory/prose file.
...
```bash
node src/commands/flint-compress.js <absolute_or_relative_filepath> --check --local-only
```
...
Default write creates `.flint/backups/<timestamp>/FILE` and report under `.flint/reports/`. Legacy direct compression also preserves `FILE.original.md` when absent. `--restore` restores latest backup or legacy original.
...
Never compress `*.original.md`, `*.backup.md`, `.flint/backups/**`, `.git/**`, `node_modules/**`, binary/archive/db/lock files, or known secret paths.
